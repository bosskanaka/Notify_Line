 ...
 */
 class Connection {
    ...
    public static int sendData(LineParameter lineData, final String TOKEN) throws IOException {
        ...
        int statusCode = 500;
        HttpURLConnection conna = preparedProperty(TOKEN);
        conna.setRequestProperty("charset", "utf-8");
        conna.setRequestProperty("Content-Length", Integer.toString(postDataLength));
        conna.setUseCaches(false);
        try  (DataOutputStream wr = new DataOutputStream(conna.getOutputStream())) {
            wr.write(postData);
            wr.flush();
            wr.close();
        }catch (Exception ex)
        ...
        if (conna.getResponseCode() != HttpURLConnection.HTTP_CREATED) {
             statusCode = conna.getResponseCode();
            if (statusCode != 200) {
